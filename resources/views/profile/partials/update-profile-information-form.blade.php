    ...
    <div class="bg-white rounded-2xl shadow-sm ring-1 ring-gray-200 overflow-hidden">
        <form method="post" action="{{ route('profile.update') }}" class="divide-y divide-gray-100">
            ...
            <div class="p-6 sm:p-10 space-y-8">
                <div
                    class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-100 pb-6">
                    <div>
                ...
                <div x-data="{ selectedAvatar: '{{ old('avatar', $user->avatar ?? '') }}' }">
                    ...
                    <div class="grid grid-cols-3 sm:grid-cols-5 gap-4">
                        @foreach ($avatarOptions as $key => $label)
                            <label
                                class="group relative cursor-pointer rounded-xl border p-2 flex flex-col items-center justify-center gap-2 transition-all duration-200 hover:bg-gray-50 hover:border-blue-200"
                                :class="selectedAvatar === '{{ $key }}' ?
                                    'ring-2 ring-[#1b3985] border-transparent bg-blue-50/30' : 'border-gray-200'">
                                <img src="{{ asset(
                                    'avatars/' .
                                        match ($key) {
                                    ...
                                    alt="{{ $label }}"
                                    class="h-12 w-12 sm:h-16 sm:w-16 transition-transform group-hover:scale-110">
                                <input type="radio" name="avatar" value="{{ $key }}" class="sr-only"
                                    x-model="selectedAvatar">
                                <span
                                    class="text-xs font-semibold text-gray-600 group-hover:text-[#1b3985]">{{ $label }}</span>

                                <div x-show="selectedAvatar === '{{ $key }}'"
                                    class="absolute top-1 right-1 text-[#1b3985]">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="mt-2 flex items-center gap-2 text-sm text-amber-600 bg-amber-50 p-2 rounded-md">
                                ...
                                <div>
                                    ...
                                    <button form="send-verification"
                                        class="underline font-semibold hover:text-amber-800">Kirim ulang.</button>
                                </div>
                            </div>
                            @if (session('status') === 'verification-link-sent')
                                <p class="mt-2 text-sm text-green-600 font-medium">Tautan verifikasi baru telah dikirim.
                                </p>
                            @endif
                        @endif
                    </div>
                    ...
                    @if ((auth()->user()->role ?? null) === 'verifikator')
                        <div class="col-span-1 md:col-span-2">
                            ...
                            <div
                                class="mt-1 flex items-center gap-2 px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-gray-500 text-sm">
                ...
                <div class="p-6 sm:p-10 bg-slate-50/50 space-y-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="col-span-1 md:col-span-2">
                            ...
                            {{-- Hapus atribut :disabled sepenuhnya agar bisa diedit oleh siapa saja --}}
                            <x-text-input id="nim" name="nim" type="text"
                                class="mt-1 block w-full bg-white" :value="old('nim', $user->nim)" />
                        ...
                        <div>
                            ...
                            <x-text-input id="tanggal_lahir" name="tanggal_lahir" type="date"
                                class="mt-1 block w-full" :value="old('tanggal_lahir', optional($user->tanggal_lahir)->format('Y-m-d'))" />
                        </div>
                        ...
                        <div>
                            ...
                            <div class="relative mt-1">
                                ...
                                <x-text-input id="nomor_hp" name="nomor_hp" type="tel"
                                    class="pl-12 block w-full" :value="old('nomor_hp', $user->nomor_hp)" placeholder="812xxxx" />
                            </div>
                        </div>
                        <div>
                            <x-input-label for="angkatan" value="Tahun Angkatan" />
                            <x-text-input id="angkatan" name="angkatan" type="number" min="2000"
                        ...
                        <div class="col-span-1 md:col-span-2">
                            ...
                            <select id="prodi_id" name="prodi_id"
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-[#1b3985] focus:ring-[#1b3985] bg-white py-2.5">
            ...
            <div class="px-6 sm:px-10 py-4 bg-gray-50 flex items-center justify-between">
                ...
                <div class="flex items-center gap-4">
                    @if (session('status') === 'profile-updated')
                        <span x-data="{ show: true }" x-show="show" x-transition.opacity.duration.1000ms
