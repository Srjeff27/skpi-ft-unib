...
<form method="post" action="{{ route('profile.update') }}" class="space-y-8" enctype="multipart/form-data">
            ...
            <div x-data="{ photoName: null, photoPreview: null }">
                ...
                <div class="mt-2 flex items-center gap-5">
                    <div class="flex-shrink-0">
                        <img x-show="!photoPreview" src="{{ $user->profile_photo_path ? asset('storage/' . $user->profile_photo_path) : 'https://ui-avatars.com/api/?name=' . urlencode($user->name) . '&background=1b3985&color=fff' }}" alt="Foto Profil" class="h-20 w-20 rounded-full object-cover">
                        <div x-show="photoPreview" class="h-20 w-20 rounded-full bg-cover bg-center" :style="'background-image: url(\'' + photoPreview + '\');'" x-cloak></div>
                    </div>
                    <div>
                        <input id="photo" name="photo" type="file" accept="image/*" class="hidden" x-ref="photo" @change="
                            photoName = $refs.photo.files[0].name;
                            const reader = new FileReader();
                            reader.onload = (e) => { photoPreview = e.target.result; };
                            reader.readAsDataURL($refs.photo.files[0]);
                        ">
                        <x-secondary-button type="button" @click.prevent="$refs.photo.click()">Ganti Foto</x-secondary-button>
        ...
        <div class="mt-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <x-text-input id="tanggal_lahir" name="tanggal_lahir" type="date" class="mt-1 block w-full" :value="old('tanggal_lahir', $user->tanggal_lahir)" />
                </div>
            </div>
            <div>
    ...
    <div class="bg-white sm:rounded-lg pt-8 mt-8 border-t">
        <header>
            <h2 class="text-lg font-bold text-gray-800">Data Kelulusan (Khusus Admin)</h2>
        ...
        <div class="mt-6 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <x-text-input id="gelar_id" name="gelar_id" type="text" class="mt-1 block w-full" :value="old('gelar_id', $user->gelar_id)" :disabled="auth()->user()->role !== 'admin'" placeholder="S.T."/>
                </div>
            </div>
            <div>
                <x-input-label for="gelar_en" value="Gelar (EN)" />
                <x-text-input id="gelar_en" name="gelar_en" type="text" class="mt-1 block w-full" :value="old('gelar_en', $user->gelar_en)" :disabled="auth()->user()->role !== 'admin'" placeholder="B.Eng."/>
            </div>
        </div>
    </div>
    @endif
