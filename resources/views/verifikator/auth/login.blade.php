<x-guest-layout>
    <div class="text-center mb-8 space-y-4">
        ...
        <div class="space-y-1">
            <h1 class="text-3xl font-extrabold text-[#0A2E73] tracking-tight">Masuk Verifikator</h1>
            <p class="text-sm text-gray-500">Gunakan akun verifikator untuk melanjutkan</p>
        </div>
    </div>

    <x-auth-session-status class="mb-4" :status="session('status')" />

    <form method="POST" action="{{ route('verifikator.login') }}" class="space-y-6">
        @csrf
        <div>
            ...
            <div class="relative">
                ...
                <input id="email" name="email" type="email" autocomplete="username" required autofocus
                    value="{{ old('email') }}"
                    class="block w-full rounded-xl border-gray-200 bg-white/80 pl-10 pr-3 py-2.5 text-gray-800 shadow-sm focus:border-[#0A2E73] focus:ring-[#0A2E73] transition" placeholder="nama@unib.ac.id">
            </div>
