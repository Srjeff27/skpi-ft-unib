<x-guest-layout>
    <div class="text-center mb-6">
        ...
        <h1 class="mt-3 text-2xl font-semibold text-black">Masuk Verifikator</h1>
        <p class="mt-1 text-sm text-gray-600">Gunakan akun verifikator untuk melanjutkan</p>
    </div>

    <x-auth-session-status class="mb-4" :status="session('status')" />

    <form method="POST" action="{{ route('verifikator.login') }}" class="space-y-5">
        @csrf
        <div>
