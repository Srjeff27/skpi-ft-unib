...
<x-guest-layout>
    ...
    @if ($isMaintenance)
        <div class="rounded-xl border-2 border-dashed border-amber-200 bg-amber-50 p-6 text-center mt-4">
            <div class="flex justify-center items-center mx-auto w-12 h-12 rounded-full bg-amber-100 text-amber-600 mb-4">
                <x-heroicon-o-wrench-screwdriver class="w-7 h-7" />
            </div>
            <h3 class="text-lg font-semibold text-amber-800">Sistem Dalam Perbaikan</h3>
            <p class="mt-1 text-sm text-amber-700">
                Aplikasi sedang dalam mode pemeliharaan. Untuk sementara, pendaftaran akun baru dinonaktifkan.
            </p>
        </div>
        <div class="relative my-7">
            <div class="absolute inset-0 flex items-center" aria-hidden="true"><div class="w-full border-t border-gray-200"></div></div>
            <div class="relative flex justify-center"><span class="bg-white px-3 text-xs font-semibold uppercase tracking-wider text-gray-400">Atau</span></div>
        </div>
        <div class="text-center">
            <a href="{{ route('login') }}" class="w-full inline-flex items-center justify-center rounded-xl border border-transparent bg-blue-50 px-4 py-3 font-semibold text-[#1b3985] hover:bg-blue-100 transition-colors">
